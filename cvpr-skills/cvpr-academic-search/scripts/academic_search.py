...
# ///
"""OpenAlex discovery utility for cvpr-academic-search (stdlib only).

This is one explicitly selectable discovery source, not an automatic fallback
or a substitute for the skill's required multi-source coverage. It is a single
self-contained file that calls a public HTTP API with no third-party
dependencies.
...
Usage:
    ...
    python3 academic_search.py --author "Alex Kim"
    # By author + topic within that author's works
    python3 academic_search.py "robust representation learning" --author "Alex Kim" --sort publication_date
    # By author CONSTRAINED to an institution (disambiguates common/romanised names)
    python3 academic_search.py --author "Alex Kim" --affiliation "Example University"
    # List every same-name author cluster, then pick the right institution / ID
    python3 academic_search.py --author "Alex Kim" --list-authors
...
Be polite to the OpenAlex pool: pass --mailto name@institution.edu or set the
...
on stderr and exit non-zero. The caller must record the failure and decide
whether the declared coverage standard is still met.
"""
...
MAILTO = "cvpr-skills@users.noreply.github.com"
...
# relevance_score can carry a citation boost, so weakly matching but highly cited
# papers may move ahead of direct matches after local re-ranking. This conservative
# floor keeps candidates reasonably close to the best text match; callers must
# still perform the Skill's full relevance and evidence review.
RELEVANCE_FLOOR = 0.3
...
def _aff_match(author: dict, affiliation: str) -> bool:
    ...
    target institution from hijacking the result.
    """
...
def _cluster_summary(results: list[dict]) -> list[dict]:
    ...
    Lets callers see distinct people behind a colliding name instead of silently
...
def resolve_author(name: str, affiliation: str | None = None) -> dict | None:
    ...
    OpenAlex often splits one real person into several records, and common names
    can collide across unrelated people. We therefore:
...
def main():
    parser = argparse.ArgumentParser(description="CVPR academic literature discovery via OpenAlex")
