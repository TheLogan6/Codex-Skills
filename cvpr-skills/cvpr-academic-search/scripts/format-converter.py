...
Outputs: .nbib (PubMed only), .ris, .bib.

Usage:
  python format-converter.py --pmid 27898976
  python format-converter.py --pmid 27898976,29234807 --format ris
  python format-converter.py --doi 10.1109/CVPR.2016.90 --format bib
  python format-converter.py --arxiv 1706.03762 --format ris
  python format-converter.py --query "deep learning AND retinal imaging"
...
refs.txt format:
  ...
  ARXIV:1706.03762
  QUERY:deep learning AND retinal imaging
  AUTHOR:Gulshan TITLE:diabetic retinopathy deep learning
...
def self_test():
    ...
    # 2. PubMed endpoint (known deep-learning paper: PMID 27898976)
    pmid = "27898976"
    ...
    try:
        ...
        if nbib_text and nbib_text.strip():
            ...
            if ris_content.strip() and bib_content.strip():
                print(f"  [OK] PubMed endpoint (RIS: {len(ris_content)}B, BibTeX: {len(bib_content)}B)")
            else:
    ...
    try:
        ...
        if ris_content.strip() and bib_content.strip():
            print(f"  [OK] CrossRef endpoint (RIS: {len(ris_content)}B, BibTeX: {len(bib_content)}B)")
        else:
...
def main():
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Examples:
  %(prog)s --pmid 27898976
  %(prog)s --pmid 27898976,29234807 --format ris
  %(prog)s --doi 10.1109/CVPR.2016.90 --format bib
  %(prog)s --doi 10.1109/CVPR.2016.90,10.1145/3065386 --format ris
  %(prog)s --arxiv 1706.03762 --format bib
  %(prog)s --arxiv 1706.03762,2302.13971 --format ris
  %(prog)s --query "deep learning AND retinal imaging"
...
refs.txt format:
  ...
  # Lines starting with # are comments
    ...
    parser.add_argument(
        "--format", choices=["nbib", "ris", "bib"], default="nbib",
        help="Output format: nbib (default, MEDLINE), ris (EndNote/Zotero), bib (BibTeX/LaTeX)",
    )
    parser.add_argument(
    ...
    if args.preflight:
        ...
        if not all_ok:
            print("  Affected: format-converter downloads for unreachable endpoints.")
            return 1
        return 0
    ...
    fmt = args.format
    if fmt == "nbib" and (args.doi or args.arxiv):
        parser.error(
            ".nbib 只适用于 PubMed/NLM 记录；DOI 或 arXiv 输入必须由用户明确选择 --format ris 或 --format bib"
        )
