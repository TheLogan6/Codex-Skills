...
Supports: MEDLINE/.nbib -> RIS/BibTeX, CrossRef JSON -> RIS/BibTeX, arXiv XML -> RIS/BibTeX.
...
def convert_from_medline(nbib_text, fmt):
    """Convert MEDLINE/.nbib text to RIS or BibTeX."""
...
def convert_from_crossref(json_data, fmt):
    """Convert CrossRef JSON to RIS or BibTeX."""
    if fmt == "ris":
...
def convert_from_arxiv(xml_root, fmt):
    """Convert arXiv Atom XML to RIS or BibTeX."""
    if fmt == "ris":
...
def get_extension(fmt):
    return {"nbib": ".nbib", "ris": ".ris", "bib": ".bib"}.get(fmt, ".nbib")
